...
- Lighthouse CI (learned.md Day 9): perf ≥ 55 on home, ≥ 72 on case pages (CI ship-floor, D71; local target remains 75+); a11y ≥ 95, SEO ≥ 95 on all 6 pages.
...
| D77 | 2026-07-12 | L0 PaperTexture baked to static `/images/l0-paper.webp` (pure procedural paper, no image overlay); PageBackground renders it as a plain background-image div; live-shader code preserved behind `LIVE_SHADER` flag for rollback | Sid's written verdict: live shader cost ~12 lighthouse points at steady state to draw a static texture (cases 60–63 → 75/75 after bake); the old 1×1 graphite `image` default also rendered as a centered black-square artifact |

| D78 | 2026-07-13 | M5.9/M5.12: full-bleed hero backdrops via Section `backdrop` prop — stops the 1200px text column from clamping PaperInk/GridBackdrop; text lives in `z-[20]` above the backdrop in `absolute inset-0` | STOP M4-A reject #1 (Sid screenshot 2026-07-12); full-bleed delivered with existing media (PaperInk / GridBackdrop) — no new deps or assets |
| D79 | 2026-07-13 | M5.10: lanyard fallback delay-show (400ms) + offset crossfade — fallback fades out over 200ms completing before canvas fades in over 300ms with 250ms delay; fast loads never show fallback | STOP M4-A reject #2: Sid saw two cards at once; server/hydration render byte-identical (client-only timers) |
| D80 | 2026-07-13 | M6.1: lanyard scene-ready fires from Band's first useFrame (post-GLTF/Texture resolve) instead of an independent FirstFrameGate; explicit sceneError state forces the designed fallback on boundary error or WebGL context loss | M3-A wash was the Environment painting before the model loaded; FirstFrameGate fired on rAF tick while WASM/GLTF still loading; sceneError prevents dead white region |
| D81 | 2026-07-13 | M6.2: knowme full-bleed card layer — canvas `absolute inset-0 pointer-events-none`, R3F eventSource on `knowme-root`, card anchored right via `anchorX=3`; text-hugging rounded scrim panel replaces the full-width inset-0 slab | STOP M4-A reject origin: translateX(18%) clipped the card at an invisible wall and canvas intercepted pointer events; Sid's spec: card drags across the whole page, no invisible walls |
| D82 | 2026-07-13 | M6.3: case heroes replaced by static ImageBackdrop server component — bg-graphite copy + token gradient overlay; PaperInk files preserved for rollback; Sid overwrites two webp files for final art with zero code change | Sid: "not 4 light illumination blobs and cursor illuminates"; shader cost was ~12 lighthouse points for a texture effect; file-slot design enables post-launch art swap |
